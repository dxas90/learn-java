...
APP_NAME="learn-java"
NAMESPACE="default"
SERVICE_NAME="learn-java"  # Helm release name
DEPLOYMENT_NAME="learn-java"  # Helm release name
TIMEOUT_SECONDS=300
RETRY_INTERVAL=5
...
validate_k8s_resources() {
    ...
    # Helm uses app.kubernetes.io/name={chart}
    kubectl get pods -l app.kubernetes.io/name=learn-java

    # Verify all pods are running
    local pod_count=$(kubectl get pods -l app.kubernetes.io/name=learn-java --field-selector=status.phase=Running --no-headers | wc -l)
    if [[ ${pod_count} -gt 0 ]]; then
        log_success "All application pods are running (${pod_count})"
    else
        log_error "No running pods found"
        kubectl describe pods -l app.kubernetes.io/name=learn-java
        exit 1
    fi
...
test_health_probes() {
    ...
    local ready_pods=$(kubectl get pods -l app.kubernetes.io/name=learn-java -o jsonpath='{.items[*].status.conditions[?(@.type=="Ready")].status}')
...
test_resource_constraints() {
    ...
    local pod_name=$(kubectl get pods -l app.kubernetes.io/name=learn-java -o jsonpath='{.items[0].metadata.name}')
...
test_pod_resilience() {
    ...
    sleep 5
    kubectl wait --for=condition=Ready pods -l app.kubernetes.io/name=learn-java --timeout=120s
